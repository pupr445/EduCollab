...
export default function CreateClassForm({ groupLabel = "Kelas" }: { groupLabel?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-4 font-medium text-slate-900">Buat {groupLabel} Baru</h2>

      <label className="mb-1 block text-sm font-medium text-slate-700">Nama {groupLabel}</label>
      <input
      ...
      <div className="flex gap-2">
        <button type="submit" disabled={loading} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
          {loading ? "Menyimpan..." : `Simpan ${groupLabel}`}
        </button>
